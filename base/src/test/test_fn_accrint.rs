...
fn fn_accrint_basis_2_actual_360() {
    ...
    // For issue 2020-1-1 to settlement 2020-3-1: 60 actual days.
    // Period 2020-1-1 to 2020-7-1: 182 actual days.
    // AI = 1000 * 0.05 * (60 / 182) = 16.483516... (same fraction as
    // basis 1 because numerator and denominator both use actual days
    // before normalization to 360).
    let mut model = new_empty_model();
    model._set(
        "A1",
    ...
    assert!(
        (v - 16.483_516_483_5).abs() < 1e-6,
        "basis 2 = {v}, expected ~16.4835 (60/182 * 50)"
    );
}
...
fn fn_accrint_basis_3_actual_365() {
    ...
    assert!(
        (v - 16.483_516_483_5).abs() < 1e-6,
        "basis 3 = {v}, expected ~16.4835 (60/182 * 50)"
    );
}
